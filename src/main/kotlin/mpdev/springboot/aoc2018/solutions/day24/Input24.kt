...
object Input24 {
    ...
    private fun processGroup(state: Int, line: String, immuneSystem: MutableMap<Int,Army>, infection: MutableMap<Int,Army>) {
        ...
        try {
            ...
            if (state == 1)
                immuneSystem[++immuneGrpId] = Army("IMM.$immuneGrpId", units.toInt(), hitPoints.toInt(), weak, immune, Pair(Immunity.fromString(kindOfDamage), damage.toInt()), initiative.toInt())
            else
                infection[++infectionGrpId] = Army("INF.$infectionGrpId", units.toInt(), hitPoints.toInt(), weak, immune, Pair(Immunity.fromString(kindOfDamage), damage.toInt()), initiative.toInt())
        }
        catch(e: Exception) {
